...
import { IconLobby, IconClapper, IconStarBurst, IconUsers, IconSeat } from "@/components/nav/NavIcons";
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 pb-[calc(0.5rem+env(safe-area-inset-bottom))] md:hidden">
      <div className="mx-auto mt-2 max-w-lg px-3">
        <ul className="grid grid-cols-5 rounded-2xl border border-[rgba(232,200,106,0.12)] bg-[#12101c]/94 p-1.5 shadow-[0_-8px_40px_rgba(0,0,0,0.45)] backdrop-blur-xl">
            ...
            return (
              <li key={href}>
                <Link
                  href={href}
                  className={cn(
                    "flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-xl py-2 text-[11px] font-semibold tracking-wide transition-colors",
                    active
